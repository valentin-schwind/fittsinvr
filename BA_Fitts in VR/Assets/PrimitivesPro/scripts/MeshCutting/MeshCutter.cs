...
//                MeshUtils.Log("Allocating vertices: " + verticesNum);
            ...
            else
            {
                ...
                cutTris.Clear();
                for (var i = 0; i < triCache.Length; i++)
                {
                    triCache[i] = 0;
                }
            }
...
//            UnityEngine.Debug.Log("Empty cut! " + vertices[0].Count + " " + vertices[1].Count);
        ...
        private int AddIntersectionPoint(Vector3 pos, Triangle tri, int edge0, int edge1, Dictionary<long, int> cache, List<Vector3> vertices, List<Vector3> normals, List<Vector2> uvs)
        {
