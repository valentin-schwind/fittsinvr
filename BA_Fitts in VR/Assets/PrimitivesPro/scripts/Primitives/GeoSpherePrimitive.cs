    ...
    public class GeoSpherePrimitive : Primitive
    {
        ...
        public static float GenerateGeometry(Mesh mesh, float radius, int subdivision, BaseType baseType, NormalsType normalsType, PivotPosition pivotPosition)
        {
            ...
            for (var i = 0; i < subdivision; i++)
            {
                ...
                for (var triIdx = 0; triIdx < triCount; triIdx += 3)
                {
                    // get triangle
        ...
        private static void InitBasePrimitive(float radius, BaseType baseType, Vector3[] vertices, Vector2[] uvs, int[] triangles)
        {
            switch (baseType)
            {
        ...
        private static int GetTriCount(BaseType type, int subdivision)
        {
            var baseTriCount = 0;
            switch (type)
            {
        ...
        private static int GetVertCount(BaseType type, int subdivision, bool sharedVertices)
        {
        ...
        private static void CorrectSeam(List<Vector3> vertices, List<Vector2> uvs, List<int> triangles)
        {
            ...
            for (var i = triangles.Count - 3; i >= 0; i -= 3)
            {
