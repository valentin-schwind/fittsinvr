...
public class CP_SSSSS_Object : MonoBehaviour {
	...
	private void SubstituteMaterial()
	{
		if (r == null) r = GetComponent<Renderer>();
		if (r != null) {
	...
	private void RevertMaterial(Camera cam)
	{
...
{
	private string[] maskSourceNames = { "Main texture from current material (A)", "Separate texture (A)", "No mask, whole object is translucent" };
	private SerializedObject e_object;
	private SerializedProperty e_subsurfaceColor;
	private SerializedProperty e_maskSource;
	...
	{
		var myScript = target as CP_SSSSS_Object;
		if (e_object == null)
		{
			e_object = new SerializedObject(target);
			e_subsurfaceColor = e_object.FindProperty("subsurfaceColor");
		...
		var msksrc = (CP_SSSSS_MaskSource)EditorGUILayout.Popup("Subsurface mask source:", (int)myScript.maskSource, maskSourceNames);
